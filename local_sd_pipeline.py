...
from register_attention_control import register_attention_control # chenchen
...
class LocalStableDiffusionPipeline(StableDiffusionPipeline):
    ...
    def __call__(
        ...
        # Register the attention control if the controller is provided
        if attn_mask is None:
            print("No mask is used.")
            if controller is not None:
                print("Storing attention scores.")
                register_attention_control(self, controller)
        else:
            print("Using masked magnitude, without storing attention scores again.")
        ...
        if m_trigger==True and l_target_pr is not None:
            original_prompt_embeds = self._encode_prompt(
                prompt,
                device,
                num_images_per_prompt,
                do_classifier_free_guidance,
        ...
        if track_noise_norm is True:
            uncond_noise_norm = []
            text_noise_norm = []
            text_noise_norm_masked = []

            for i in range(len(latents)):
                uncond_noise_norm.append([])
                text_noise_norm.append([])
                text_noise_norm_masked.append([])
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if do_classifier_free_guidance:
                    # w/ PR
                    if m_trigger==True and l_target_pr is not None:
                        noise_pred_uncond, noise_pred_text_modified = noise_pred.chunk(2)
                        _, noise_pred_text_original = original_noise_pred.chunk(2)
                        mag_curr = (noise_pred_text_modified-noise_pred_uncond).norm(p=lp).item()
                        # print(f"t = {i}, mag_curr = {mag_curr}")
                        if mag_curr > l_target_pr:
                            scaler = (guidance_scale * l_target_pr - mag_thres_pr) / (guidance_scale - 1)
                            s_pr =  min((mag_curr-scaler)/(mag_thres_pr-scaler)*guidance_scale, guidance_scale)
                            noise_pred_text = noise_pred_text_modified - noise_pred_text_original
                            noise_pred = noise_pred_text_original + s_pr * noise_pred_text + (guidance_scale - s_pr) * (noise_pred_text_modified - noise_pred_uncond)
                        else:
                            noise_pred_text = noise_pred_text_modified - noise_pred_uncond
                    ...
                    if attn_mask is not None and track_noise_norm is True:
                        noise_pred_text_masked = noise_pred_text * attn_mask #chenchen
                        attn_mask_mean = attn_mask.mean(dim=(1,2,3)).float()#torch.mean(attn_mask.float()).item()
                        #del attn_mask
                        torch.cuda.empty_cache()
                    else:
                        noise_pred_text_masked = noise_pred_text #chenchen
                        attn_mask_mean = None
                        torch.cuda.empty_cache()
                ...
                if track_noise_norm is True:
                    for j in range(len(uncond_noise_norm)):
                        uncond_noise_norm[j].append(noise_pred_uncond[j].norm(p=lp).item())
                        text_noise_norm[j].append(noise_pred_text[j].norm(p=lp).item())
                        if attn_mask_mean is not None:
                            text_noise_norm_masked[j].append(noise_pred_text_masked[j].norm(p=lp).item() / attn_mask_mean[j].item())
                        else:
                            text_noise_norm_masked[j].append(noise_pred_text_masked[j].norm(p=lp).item()) #chenchen
        ...
        if track_noise_norm is True:
            track_stats = {
                ...
                "text_noise_norm_masked": text_noise_norm_masked,
            }
            return (
    ...
    def aug_prompt(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i in target_steps:
                    single_prompt_embeds = prompt_embeds[[-1], :, :].clone().detach() # shape [1,77,768]
                    original_prompt_embeds = single_prompt_embeds.clone() #PRSS
                    if print_optim is True or optim_epsilon is not None:
                        init_embeds = single_prompt_embeds.clone()
                    single_prompt_embeds.requires_grad = True
                    dummy_prompt_embeds = prompt_embeds[[0], :, :].clone() # shape [1,77,768]
                    ...
                    for j in range(optim_iters):
                        ...
                        if attn_mask is not None:
                            noise_pred_text_masked = noise_pred_text * attn_mask #chenchen
                            noise_pred_text_norm = torch.norm(noise_pred_text_masked, p=2).mean() / torch.mean(attn_mask.float())
                        else:
                            noise_pred_text_norm = torch.norm(noise_pred_text, p=2).mean()
                        loss = noise_pred_text_norm
                        loss_item = loss.item()
                        ...
                        if attn_token_mask is not None:
                            scaling_factor = 77 / np.sum(attn_token_mask[0, 1:])
                            attn_token_mask = attn_token_mask * scaling_factor
                            attn_token_mask_expanded = np.expand_dims(attn_token_mask, axis=-1)  
                            attn_token_mask_expanded = np.tile(attn_token_mask_expanded, (1, 1, 768)) 
                            attn_token_mask_expanded = torch.tensor(attn_token_mask_expanded, dtype=single_prompt_embeds.grad.dtype).to(device)

                            single_prompt_embeds.grad = single_prompt_embeds.grad * attn_token_mask_expanded
                    ...
                    torch.cuda.empty_cache()
                    return single_prompt_embeds, original_prompt_embeds, m_trigger #PRSS
    ...
    def aug_prompt_ss(
        ...
        if ss_prompt is not None:
            prompt_embeds_ss_lst = [prompt_embeds]
            for ith in range(len(ss_prompt)):
                prompt_embeds_ss = self._encode_prompt(
                    ss_prompt[ith],
                    device,
                    num_images_per_prompt,
                    do_classifier_free_guidance,
                ...
                prompt_embeds_ss_lst.append(prompt_embeds_ss)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i in target_steps:
                    mag_ss_lst = []
                    for j in range(len(prompt_embeds_ss_lst)):
                        prompt_embeds_curr = prompt_embeds_ss_lst[j]
                        single_prompt_embeds = prompt_embeds_curr[[-1], :, :].clone().detach() 
                        if j == 0:
                            original_prompt_embeds = single_prompt_embeds.clone() 
                        if print_optim is True or optim_epsilon is not None:
                            init_embeds = single_prompt_embeds.clone()
                        ...
                        if target_loss is not None:
                            if loss_item <= target_loss:
                                if print_optim is True:
                                    print(f"step: {j}, curr mag: {loss_item}")
                                    if j != 0:
                                        print(f"Prompt: {ss_prompt[j-1]}")
                                    else:
                                        print("Using original prompt.")
                                break
                            else: 
                                m_trigger = True 
                                mag_ss_lst.append(loss_item)

                        if print_optim is True:
                            print(f"prompt number: {j}, curr mag: {loss_item}")

                        if len(mag_ss_lst) == len(prompt_embeds_ss_lst):
                            best_ss_idx = np.argmin(mag_ss_lst)
                            if print_optim is True:
                                print("All above thres. Using the {}th alternative, which has the min mag of {}.".format(best_ss_idx, np.min(mag_ss_lst)))
                                print(f"Prompt: {ss_prompt[best_ss_idx-1]}")
                            single_prompt_embeds = prompt_embeds_ss_lst[best_ss_idx][[-1], :, :].clone().detach()
                            break
